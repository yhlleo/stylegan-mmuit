...
parser.add_argument('--save_dir', type=str, default='./styleflow-results')
...
for i in tqdm(range(1000)):
    ...
    img_org, _ = gan_model([w_src], input_is_latent=True, randomize_noise=True)
    img_org = torch.clamp(img_org * 0.5 + 0.5, 0, 1)
    vutils.save_image(img_org.data, os.path.join(args.save_dir, '{:04d}.jpg'.format(i)), padding=0)

    final_array_target = torch.cat([light, attr], dim=1)
    zero_padding = torch.zeros(1, 18, 1).to(device)
    for idx, idy in zip([0,1,6,7], [0,1,2,3]):
        ...
        vutils.save_image(GAN_image.data, os.path.join(args.save_dir, '{:04d}-{}-{}.jpg'.format(i, idy, 0)), padding=0)

        w_current = rev1[0]
        GAN_image, _ = gan_model([w_current], input_is_latent=True, randomize_noise=True)
        GAN_image = torch.clamp(GAN_image * 0.5 + 0.5, 0, 1)
        vutils.save_image(GAN_image.data, os.path.join(args.save_dir, '{:04d}-{}-{}.jpg'.format(i, idy, 1)), padding=0)
