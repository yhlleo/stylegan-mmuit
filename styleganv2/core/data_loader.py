...
class FFHQImageDataset(Dataset):
    def __init__(self, 
        ...
        all_images        =  glob.glob(os.path.join(source_path, mode, "*.png"))
        all_images.sort()
...
class Cat2DogImageDataset(Dataset):
    def __init__(self, 
        source_path, 
        attr_path, 
        latent_path,
        ...
        self.source_path   = source_path
        self.latent_path   = os.path.join(latent_path, mode, '{}.npy'.format(latent_type))
        self.attr_path     = os.path.join(attr_path, "list_cat2dog-{}.txt".format(mode))
        self.transform     = transform
        self.selected_attrs = attributes
        self.mode          = mode
        self.num_selected_attrs = len(self.selected_attrs)
    ...
    def preprocess(self, attr_path):
        ...
        for ll in lines:
            splits = ll.split()[:2]
            fname, label = splits[0], int(splits[1])
            self.image2attr[fname] = label
