...
def translate_reconstruction(nets, model, args, img_src, lat, lab_trg, filename, device):
    N = img_src.size(0)
    out_concat = [img_src]
    lat = model.preprocess(lat, latent_space_type=args.latent_type)
    ...
    for i in range(3):
        ...
        lat_out = model.preprocess(lat_out, latent_space_type=args.latent_type)
        img_trg = model.synthesize(lat_out, latent_space_type=args.latent_type)["image"]
