...
def detect_face(imgs, minsize, pnet, rnet, onet, threshold, factor, device):
    ...
    #imgs = imgs.permute(0, 3, 1, 2).type(model_dtype) # NCHW
...
def imresample(img, sz):
    im_data = interpolate(img, size=sz, mode="bilinear")
    return im_data
...
def save_img(img, path):
    img = (img+1.0)/2.0
    img.clamp_(0, 1)
    vutils.save_image(img.data, path, padding=0)
    #if isinstance(img, np.ndarray):
...
def extract_face(img, box, image_size=160, margin=0, save_path=None):
    ...
    Arguments:
        ...
        img {torch.Tensor} -- A torch tensor with shape [N, C, H, W]
