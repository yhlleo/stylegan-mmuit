...
parser.add_argument('--save_dir', type=str, default='./results-sample')
...
def main(args):
  """Main function."""
  attribute_path = "/path/to/datasets/styleflow/list_attr_celeba-test.txt"
  ...
  save_dir = args.save_dir #"ours-results-sample"
  ...
  for idx in [0]:
    ...
    for i in tqdm(range(start_idx, end_idx)):
      ...
      for idy in range(args.num_sample):
        ...
        vutils.save_image(fake_pos.data, os.path.join(save_dir, '{:04d}-{}-1.jpg'.format(i, idy)), padding=0)
        vutils.save_image(fake_neg.data, os.path.join(save_dir, '{:04d}-{}-0.jpg'.format(i, idy)), padding=0)
